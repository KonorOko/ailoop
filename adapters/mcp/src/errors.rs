...
/// schema mismatches) are *not* in here — they are mapped to a
/// [`ailoop_core::ToolResultContent`] with `is_error: true` inside the
/// tool wrapper so the model sees the failure as a tool reply,
/// mirroring the `SubAgentTool` policy.
#[derive(Debug, Error)]
#[non_exhaustive]
pub enum McpError {
